...
[Cloud Run service](https://cloud.google.com/run/docs/overview/what-is-cloud-run#services) is a managed compute platform that lets you run containers directly on top of Google's scalable infrastructure.
...
Before creating your first Cloud Run service instance, ensure you have completed the following prerequisites:

1. **Completed Prior Stages:** Successful creation of Cloud Run service resources depends on the completion of the following stages:
...
1. **Create your configuration files:**
    ...
    * Create YAML files defining the properties of each Cloud Run service instance you want to create. Ensure these files are stored in the `configuration/consumer/CloudRun/Service/config` folder within this repository.

    * Each YAML file should map to a single Cloud Run service instance providing details such as Service name, region, and others configuration. Each field and its structure are described in the [input section](#inputs) below.

    * For reference on how to structure your Cloud Run service configuration YAML files, see the [example](#example) section below or refer to sample YAML file at folder location `configuration/consumer/CloudRun/Service/config/instance.yaml.example`. These examples provide templates that you can adapt to your specific needs.


2. **Initialize Terraform:**

    * Open your terminal and navigate to the Cloud Run service directory containing the Terraform configuration.
...
3. **Review the Execution Plan:**
    ...
    * Use the terraform plan command to generate an execution plan. This will show you the changes Terraform will make to your Google Cloud infrastructure:
    ...
    ```
    terraform plan -var-file=../../../../configuration/consumer/CloudRun/Service/cloudrunservice.tfvars
    ```
...
4. **Apply the Configuration:**
    ...
    Once you're satisfied with the plan, execute the terraform apply command to provision your Cloud Run service instances:

    ```
    terraform apply -var-file=../../../../configuration/consumer/CloudRun/Service/cloudrunservice.tfvars
    ```

Terraform will read the YAML files from the `configuration/consumer/CloudRun/Service/config` folder and create the corresponding Cloud Run instances in your Google Cloud project.
...
5. **Monitor and Manage:**
    ...
    * Use Terraform to manage updates and changes to your Cloud Run service instances as needed.

### Example

To help you get started, we've provided examples of YAML configuration files that you can use as templates for your Cloud Run service instances.
...
4. Creating Cloud Run service using Direct VPC Egress:
    ```
    project_id: <your-project-id>
    region: us-central1
    name: service4
...
* This README is a starting point. Customize it to include specific details about your Cloud Run services and relevant configuration.
...
* Troubleshooting: If you encounter errors during the Cloud Run services creation process, verify that all prerequisites are satisfied and that the dependencies between stages are correctly configured.
...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_cloud_run_service"></a> [cloud\_run\_service](#module\_cloud\_run\_service) | github.com/GoogleCloudPlatform/cloud-foundation-fabric//modules/cloud-run-v2 | v34.1.0 |
...
| region | Cloud Run service deployment region | `string` | n/a | yes |
| name | The name of the Cloud Run service to create | `string` | n/a | yes |
| <a name="input_config_folder_path"></a> [config\_folder\_path](#input\_config\_folder\_path) | Location of YAML files holding Cloud Run service configuration values. | `string` | `"../../../../configuration/consumer/CloudRun/Service/config"` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_cloud_run_service_details"></a> [cloud\_run\_service\_details](#output\_cloud\_run\_service\_details) | Display Cloud Run service details of the Cloud Run service created. |
<!-- END_TF_DOCS -->
